...
export const handleSingleTransaction = async (
  ...
  if (record.contractFn === 'setAddr(bytes32,uint256,bytes)') {
    ...
    // use 0x00... for ARB because an empty string throws
    if (coinRecord.key === 'arb' && coinRecord.value === '') {
  ...
  try {
    ...
    const transactionArray = records.map(record => {
      ...
      if (record.contractFn === 'setAddr(bytes32,uint256,bytes)') {
        ...
        let addressAsBytes
        // use 0x00... for ARB because an empty string throws
        if (record.key === 'arb' && record.value === '') {
...
const resolvers = {
  Query: {
    publicResolver: async () => {
      try {
        const ens = getENS()
        const resolver = await ens.getAddress('arb')
        return {
          address: resolver,
          __typename: 'Resolver'
        }
    ...
    getResolverMigrationInfo: async (_, { name, resolver }) => {
      ...
      async function calculateIsPublicResolverReady() {
        const publicResolver = await ens.getAddress('arb')
