...
const resolvers = {
  Query: {
    async getRentPrice(_, { label, duration }) {
      ...
      const result = await registrar.getRentPrice(label, duration)
      return result
    },
    ...
    async getPremium(_, { name, expires, duration }) {
      const registrar = getRegistrar()
      const result = await registrar.getPremium(name, expires, duration)
      return result
    },
    async getTimeUntilPremium(_, { expires, amount }) {
      const registrar = getRegistrar()
      const result = await registrar.getTimeUntilPremium(expires, amount)
      return result
    },
